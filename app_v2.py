...
<body>
    ...
    <div class="container" id="mainContainer" style="display: none;">
        ...
        <div id="generate-panel" class="panel active">
                ...
                <div id="video-upload-panel" style="display: none;">
                    <div class="form-group">
                        ...
                        <div class="upload-area" id="videoUploadArea" onclick="document.getElementById('videoInput').click()">
                            <div class="upload-text" id="videoUploadText">
                                <div class="upload-icon">[视频]</div>
                                <h4>点击上传视频</h4>
                                <p>支持 MP4 格式，最大 50MB</p>
                            </div>
                        </div>
                        <input type="file" id="videoInput" accept="video/mp4" onchange="handleVideoUpload(event)">
                        
                        <!-- 视频预览 -->
                        <div id="videoPreview" style="display: none; margin-top: 16px;">
                            <video id="previewVideo" controls style="width: 100%; max-height: 300px; border-radius: 12px;"></video>
                            <button type="button" class="btn-secondary" onclick="clearVideo()" style="margin-top: 12px; width: auto; padding: 8px 16px; font-size: 13px;">
                                删除视频
                            </button>
                        </div>
                    </div>
            ...
            <div class="card result" id="result" style="display: none;">
                ...
                <video id="resultVideo" src="" controls style="display: none; width: 100%; border-radius: 12px; margin-bottom: 16px;"></video>
    ...
    <script>
        ...
        function setupVideoSizes() {
            const sizeSelect = document.getElementById('size');
            sizeSelect.innerHTML = `
                <option value="1080x1920" selected>竖屏 1080×1920 (9:16)</option>
                <option value="1920x1080">横屏 1920×1080 (16:9)</option>
                <option value="1024x1024">方形 1024×1024 (1:1)</option>
                <option value="720x1280">竖屏 720×1280 (9:16)</option>
            `;
        }
        ...
        async function generateImage() {
            ...
            if (currentMode === 'image' || currentMode === 'video-image') {
                ...
                if (uploadedImages.length === 0) { 
                    showError(isVideoMode ? '请上传参考图片' : '请至少上传一张参考图片'); 
                    return; 
                }
        ...
        async function generateVideo(apiKey, model, size, prompt, imageData) {
            ...
            currentVideoUrl = result.video.url;
            document.getElementById('resultVideo').src = currentVideoUrl;
        ...
        function loadHistory() {
            ...
            container.innerHTML = history.map((item, index) => `
                <div class="history-item" onclick="viewHistory(${index})">
                    ${item.isVideo ? 
                        `<video src="${item.url}" muted style="width:100%;height:120px;object-fit:cover;border-radius:8px;"></video>` :
...
class APIHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_video_generate(self):
        ...
        try:
            ...
            size = post_data.get('size', '1080x1920')
            ...
            if mode == 'image':
                ...
                if images_base64 and len(images_base64) > 0:
                    request_data['image'] = f"data:image/png;base64,{images_base64[0]}"
            
            # 视频生视频
            elif mode == 'video':
                video_base64 = post_data.get('video')
                if video_base64:
                    request_data['video'] = f"data:video/mp4;base64,{video_base64}"
